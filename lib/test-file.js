 ...
 * @private
 */
function extractBody(test262File) {
  let start = test262File.contents.indexOf(yamlStart);
  let end = test262File.contents.indexOf(yamlEnd);

  if (end > -1) {
    return test262File.contents.slice(0, start) + test262File.contents.slice(end + 5);
  }
...
class TestFile {
  constructor(file, contents) {
    if (!file) {
      throw new Error('TestFile: missing "file"');
    }

    if (!contents) {
      throw new Error('TestFile: missing "contents"');
    }
...
class Test262File extends TestFile {
  constructor(file, contents) {
    super(file, contents);
    this.contents = extractBody({contents});
    this.attrs = extractAttrs({contents, file});
    this.copyright = extractCopyright({contents});
    this.scenario = null;
